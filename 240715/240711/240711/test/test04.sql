...
-- 흐림	2
-- 구름조금	2
SELECT `weather`, count(*) AS `count` FROM `weatherhistory` GROUP BY `weather` ORDER BY `count` DESC;
...
-- 나쁨	5.4
-- 좋음	4.98
SELECT `microDust`, avg(`windSpeed`) AS `windSpeed` FROM `weatherhistory` GROUP BY `microDust` HAVING `windSpeed` > 4.0 ORDER BY `windSpeed` DESC;
